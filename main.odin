...
run_program :: proc(nes: ^NES, rom: []u8) {

    fmt.println("Running program...")

    // copy rom code to $8000
    copy(nes.ram[0x8000:0x8000+len(rom)], rom)

    // set pc to $8000
    nes.program_counter = 0x8000;

    // run clocks
    for nes.ram[nes.program_counter] != 0x00 {
        run_instruction(nes)
    }
...
do_addrmode_zp :: proc(nes: ^NES) -> u8 {
    addr_offset := nes.ram[nes.program_counter]
    nes.program_counter += 1

    res : u16 = 0x00
    res += u16(addr_offset)

    fmt.printf("ZP addressing mode. getting byte at %#X\n", res)
...
run_instruction :: proc(using nes: ^NES) {
    ...
    switch instr {
        case 0xA5: // LDA, ZP, 2, 3
            ...
            cycles += 3
        case 0x65: // ADC, ZP, 2, 3
            fmt.println("adc zp instruction")
            mem := do_addrmode_zp(nes)
            ...
            set_flag(&flags, .Overflow,
                 ((~(u16(accumulator) ~ u16(mem)) &
                 (u16(accumulator) ~ u16(temp))) & 0x0080) != 0)

            accumulator = u8(temp & 0x00FF)
            cycles += 3
        case 0x85: // STA, ZP, 2, 3
